...
export default function ExerciseFillBlank({ exercise, onAnswer }: Props) {
  ...
  const handleSubmit = () => {
    ...
    const correct = clean === exercise.correctAnswer.replace(/\s/g, '');
    onAnswer(correct);
  };

  const isCorrect = value.trim().replace(/\s/g, '') === exercise.correctAnswer.replace(/\s/g, '');
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-center">
        <input
          ...
          autoFocus
          className={`text-center text-2xl font-bold p-4 rounded-xl border-2 w-48 outline-none transition-all ${
            answered
              ? isCorrect
